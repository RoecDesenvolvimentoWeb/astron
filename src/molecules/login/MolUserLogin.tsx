...
interface IMolUserLoginProps {
    login: (data: ILoginData) => unknown;
    setIsToRecovery: () => unknown;
    setIsToLoginWithCode: () => unknown;
    isLoggingIn: boolean;
    className?: string;
}
...
type TLoginFields = "password" | "email";

export const MolUserLogin = (props: IMolUserLoginProps): ReactElement => {
    const [loginData, setLoginData] = useState<ILoginData>({
        email: "",
        password: "",
    });

    const handleLoginChange = (field: TLoginFields, value: string): void => {
        setLoginData({
            ...loginData,
            [field]: value,
        });
    ...
    return (
        <div
            className={
                "w-full md:w-[516px] h-[644px] shrink-0 inline-flex p-[44px] flex-col justify-center items-center gap-[44px] rounded-[16px] bg-[#FEFEFE] max-sm:h-[75%] max-sm:justify-start max-md:w-full max-md:h-3/5 " +
				props.className
            }>
            <AtomLoading
                isInView={props.isLoggingIn}
                ...
                <svg
                    ...
                    <defs>
                        <linearGradient
                            ...
                            <stop stopColor="#572463" />
                            <stop offset="1" stopColor="#D95DCA" />
                        </linearGradient>
                        <linearGradient
            ...
            <div className="flex flex-col items-center justify-center gap-[20px] w-full md:w-auto">
                <div className="w-full md:w-[282px]">
                    <span className="text-neutral-400 text-sm font-normal font-['Geologica'] leading-tight">
						Ainda não possui cadastro?
                    </span>
                    <span className="text-zinc-500 text-sm font-medium font-['Geologica'] leading-tight">
                        {" "}
                    </span>
                    <Link to={"/register"} className={"appearance-none"}>
                        <span className="cursor-pointer text-fuchsia-950 text-sm font-medium font-['Geologica'] underline leading-tight">
							Cadastrar
                        </span>
                    </Link>
                </div>
                <AtomForm
                    ...
                    <AtomDivLabel className="w-full md:w-fit">
                        <AtomLabel isRequired={false}>E-mail</AtomLabel>
                        <AtomInputField
                            onChange={(data): void => {
                                handleLoginChange("email", data);
                            }}
                            inputType={"email"}
                            placeHolder={"usuario@email.com"}
                            className="w-full md:w-[389px] h-[50px]"
                        />
                    </AtomDivLabel>
                    <AtomDivLabel className="w-full md:w-fit">
                        <AtomLabel isRequired={false}>Senha</AtomLabel>
                        <AtomInputField
                            onChange={(data): void => {
                                handleLoginChange("password", data);
                            }}
                            inputType={"password"}
                    ...
                    <AtomBtn
                        btnType={"submit"}
                        className={
                            "mt-3 bg-gradient-to-r from-lilas-lv1 to-lilas-lv5 w-full md:w-[373px]"
                        }>
						Entrar
                    </AtomBtn>
                </AtomForm>
                <AtomGoogleAuth
                    className="w-full md:w-[373px] font-medium"
                    type="LOGIN"/>
                <div className="w-full flex flex-row justify-between">
                    <div
                        onClick={(): void => {
                            props.setIsToRecovery();
                        }}
                        className="cursor-pointer text-fuchsia-950 text-sm font-medium font-['Geologica'] leading-tight">
						Esqueceu sua senha?
                    </div>
                    <div
                        onClick={(): void => {
                            props.setIsToLoginWithCode();
                        }}
                        className="cursor-pointer text-fuchsia-950 text-sm font-medium font-['Geologica'] leading-tight">
						Possui um código?
                    </div>
